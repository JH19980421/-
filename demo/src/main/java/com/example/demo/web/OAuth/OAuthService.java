...
import com.example.demo.web.OAuth.response.*;
...
import static com.example.demo.web.response.BaseResponseStatus.INVALID_OAUTH_TYPE;
...
public class OAuthService {
    ...
    public SocialOAuth oAuthLoginOrJoin(SocialLoginType socialLoginType, String code) throws IOException {
        ...
        switch (socialLoginType) {
            case GOOGLE: {
                ...
                if (userService.checkUserByEmail(googleUser.getEmail())) { // user가 DB에 있다면, 로그인 진행
                    // 유저 정보 조회
                    GetUserResponse getUserRes = userService.getUserByEmail(googleUser.getEmail());
                    ...
                    // 유저 정보 저장
                    PostUserResponse postUserRes = userService.createOAuthUser(googleUser.toEntity());
            ...
            case KAKAO: {
                ...
                if (userService.checkUserByEmail(kakaoUser.getEmail())) {
                    ...
                    String jwtToken = jwtService.createJwt(getUserRes.getId());
                    SocialOAuth getSocialOAuthRes = new SocialOAuth(jwtToken, getUserRes.getId(), oAuthToken.getAccessToken(), oAuthToken.getTokenType());

                    return getSocialOAuthRes;

                } else {
                    PostUserResponse postUserRes = userService.createOAuthUser(kakaoUser.toEntity());
                    SocialOAuth getSocialOAuthRes = new SocialOAuth(postUserRes.getJwt(), postUserRes.getId(), oAuthToken.getAccessToken(), oAuthToken.getAccessToken());
